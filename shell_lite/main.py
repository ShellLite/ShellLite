...
def run_repl():
    ...
    safe_mode = os.environ.get("SHL_SAFE") == "1"
    buffer = []
    while True:
        try:
...
def install_globally():
    ...
    try:
        ...
        print("\n[SUCCESS] ShellLite (v0.6.3.3) is installed!")
...
def search_package(query: Optional[str] = None):
    """Search for packages in the ShellLite ecosystem."""
    print("\n" + "=" * 50)
    print(f"  Searching for: {query if query else 'all'}")
    print("=" * 50)
    print("Notice: The package registry is being rebuilt for v0.6.3.3")
    print("In the meantime, you can find packages at: https://github.com/topics/shell-lite")
    print("=" * 50 + "\n")


def list_packages():
    """List all installed ShellLite packages."""
    home = os.path.expanduser("~")
    modules_dir = os.path.join(home, ".shell_lite", "modules")
    print("\n" + "=" * 50)
    print("  Installed ShellLite Packages")
    print("=" * 50)
    if not os.path.exists(modules_dir) or not os.listdir(modules_dir):
        print("No packages installed.")
    else:
        for pkg in sorted(os.listdir(modules_dir)):
            pkg_path = os.path.join(modules_dir, pkg)
...
def show_package_info(package_name: str):
    ...
    home = os.path.expanduser("~")
    modules_dir = os.path.join(home, ".shell_lite", "modules")
    target_dir = os.path.join(modules_dir, package_name.split("/")[-1])
    ...
    if not os.path.exists(target_dir):
        ...
        toml_path = os.path.join(target_dir, "shell-lite.toml")
        if os.path.exists(toml_path):
            with open(toml_path, "r") as f:
                print(f.read())
        else:
            print("No shell-lite.toml found for this package.")
    print("=" * 50 + "\n")
...
def install_package(package_name: str, branch: str = "main", _visited=None):
    ...
    if zip_data is None:
        print("Installation failed: No data downloaded.")
        return
...
def lint_file(filename: str):
    ...
    try:
        ...
        print(json.dumps([{"line": line, "message": str(e)}]))
...
def format_file(filename: str):
    ...
    if not os.path.exists(filename):
        print(f"Error: File '{filename}' not found.")
        return
    print("Notice: The formatting feature is being rebuilt for v0.6.3.3 and is temporarily unavailable.")
    return
...
def self_install_check():
    ...
    if not shutil.which("shl"):
        ...
        msg = "Would you like to install it so 'shl' works everywhere? (y/n): "
