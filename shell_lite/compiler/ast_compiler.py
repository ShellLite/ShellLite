...
class ASTCompiler(BaseVisitor):
    ...
    def visit_block(self, body: List[Node]) -> List[ast.stmt]:
        ...
        for stmt in body:
            ...
            if compiled:
                if isinstance(compiled, list): result.extend(compiled)
                elif isinstance(compiled, ast.expr): result.append(ast.Expr(value=compiled))
                else: result.append(compiled)
        if not result: result.append(ast.Pass())
        return result

    def visit_Number(self, node: Number): return ast.Constant(value=node.value)
    def visit_String(self, node: String): return ast.Constant(value=node.value)
    def visit_Boolean(self, node: Boolean): return ast.Constant(value=node.value)
    
    def visit_VarAccess(self, node: VarAccess):
        if node.name == "null": return ast.Constant(value=None)
    ...
    def visit_FileWrite(self, node: FileWrite):
        return ast.Expr(value=ast.Call(func=ast.Name(id="std_io_write", ctx=ast.Load()), args=[self.visit(node.path), self.visit(node.content)], keywords=[]))
    ...
    def visit_TypedAssign(self, node: TypedAssign):
        if hasattr(node, "symbol_ref") and node.symbol_ref and getattr(node.symbol_ref, "is_global", False):
            return ast.Assign(targets=[ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr=node.name, ctx=ast.Store()) if getattr(node.symbol_ref, "is_property", False) else ast.Name(id=node.name, ctx=ast.Store())], value=self.visit(node.value))
    ...
    def visit_Assign(self, node: Assign):
        if hasattr(node, "symbol_ref") and node.symbol_ref and getattr(node.symbol_ref, "is_property", False):
            return ast.Assign(targets=[ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr=node.name, ctx=ast.Store())], value=self.visit(node.value))
        return ast.Assign(targets=[ast.Name(id=node.name, ctx=ast.Store())], value=self.visit(node.value))

    def visit_Print(self, node: Print):
        return ast.Expr(value=ast.Call(func=ast.Name(id="print", ctx=ast.Load()), args=[self.visit(node.expression)], keywords=[]))
    ...
    def visit_BinOp(self, node: BinOp):
        ...
        if node.op == "+":
            if is_numeric: return ast.BinOp(left=left, op=ast.Add(), right=right)
            return ast.Call(func=ast.Name(id="mixed_concat", ctx=ast.Load()), args=[left, right], keywords=[])
        
        if node.op == ".":
            if isinstance(node.right, VarAccess): return ast.Attribute(value=left, attr=node.right.name, ctx=ast.Load())
        ...
        op_map = {"-": ast.Sub(), "*": ast.Mult(), "/": ast.Div(), "%": ast.Mod(), "==": ast.Eq(), "!=": ast.NotEq(), "<": ast.Lt(), ">": ast.Gt(), "<=": ast.LtE(), ">=": ast.GtE()}
        if node.op in ("and", "or"): return ast.BoolOp(op=ast.And() if node.op=="and" else ast.Or(), values=[left, right])
        mapped = op_map.get(node.op, ast.Eq())
        return ast.Compare(left=left, ops=[mapped], comparators=[right]) if isinstance(mapped, ast.cmpop) else ast.BinOp(left=left, op=mapped, right=right)

    def visit_UnaryOp(self, node: UnaryOp):
        right = self.visit(node.right)
        if node.op == "not": return ast.UnaryOp(op=ast.Not(), operand=right)
        if node.op == "-": return ast.UnaryOp(op=ast.USub(), operand=right)
        return right
    ...
    def visit_FunctionDef(self, node: FunctionDef):
        ...
        return ast.FunctionDef(name=node.name, args=ast.arguments(posonlyargs=[], args=[ast.arg(arg=a[0]) for a in node.args], kwonlyargs=[], kw_defaults=[], defaults=[]), body=body, decorator_list=[], returns=None)
    ...
    def visit_Call(self, node: Call):
        ...
        if node.body:
            ...
            return [body_def, ast.Expr(value=ast.Call(func=ast.Name(id=mapped.get(node.name, node.name), ctx=ast.Load()), args=args, keywords=keywords))]
            
        return ast.Call(func=ast.Name(id=mapped.get(node.name, node.name), ctx=ast.Load()), args=args, keywords=keywords)

    def visit_If(self, node: If): return ast.If(test=self.visit(node.condition), body=self.visit_block(node.body), orelse=self.visit_block(node.else_body) if node.else_body else [])
    
    def visit_Try(self, node: Try):
        return ast.Try(
            body=self.visit_block(node.try_body),
            handlers=[ast.ExceptHandler(type=ast.Name(id="Exception", ctx=ast.Load()), name=node.catch_var, body=self.visit_block(node.catch_body))],
            orelse=[],
            finalbody=[]
        )
    ...
    def visit_While(self, node: While): return ast.While(test=self.visit(node.condition), body=self.visit_block(node.body), orelse=[])
    ...
    def visit_ForIn(self, node: ForIn): return ast.For(target=ast.Name(id=node.var_name, ctx=ast.Store()), iter=self.visit(node.iterable), body=self.visit_block(node.body), orelse=[])
    def visit_Return(self, node: Return): return ast.Return(value=self.visit(node.value))
    def visit_Stop(self, node: Stop): return ast.Break()
    def visit_Skip(self, node: Skip): return ast.Continue()
    ...
    def visit_IndexAssign(self, node: IndexAssign):
        return ast.Assign(targets=[ast.Subscript(value=self.visit(node.obj), slice=self.visit(node.index), ctx=ast.Store())], value=self.visit(node.value))

    def visit_PropertyAssign(self, node: PropertyAssign):
        return ast.Assign(targets=[ast.Attribute(value=ast.Name(id=node.instance_name, ctx=ast.Load()), attr=node.property_name, ctx=ast.Store())], value=self.visit(node.value))
    ...
    def visit_ClassDef(self, node: ClassDef):
        ...
        for prop_name, default_node in node.properties:
            ...
            init_body.append(ast.Assign(targets=[ast.Attribute(value=ast.Name(id="self", ctx=ast.Load()), attr=prop_name, ctx=ast.Store())], value=ast.Name(id=prop_name, ctx=ast.Load())))
        ...
        for m in node.methods:
            ...
            body.append(ast.FunctionDef(name=m.name, args=ast.arguments(posonlyargs=[], args=m_args, kwonlyargs=[], kw_defaults=[], defaults=[]), body=m_body, decorator_list=[], returns=None))
        return ast.ClassDef(name=node.name, bases=[ast.Name(id=node.parent or "ShellLiteObject", ctx=ast.Load())], keywords=[], body=body, decorator_list=[])

    def visit_Instantiation(self, node: Instantiation):
        call = ast.Call(func=ast.Name(id=node.class_name, ctx=ast.Load()), args=[self.visit(a) for a in node.args], keywords=[])
        return ast.Assign(targets=[ast.Name(id=node.var_name, ctx=ast.Store())], value=call) if node.var_name else ast.Expr(value=call)
