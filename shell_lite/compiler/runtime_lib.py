...
def contains(obj, item): return item in obj
def split(x, sep=None): return x.split(sep) if sep else x.split()
def upper(x): return x.upper()
def lower(x): return x.lower()
def sort(x): return sorted(x)
def count(x, y=None): return x.count(y) if y is not None else len(x)
def json_parse(s): return json.loads(s)
def json_stringify(obj, indent=2): return json.dumps(obj, indent=indent)
def pop(target, index=-1): return target.pop(index)
def clear_dict(d): d.clear()
def add(target, item):
    if isinstance(target, set): target.add(item)
    else: target.append(item)
def remove(target, item): target.remove(item)

def spawn_task(func, *args, **kwargs): return __executor.submit(func, *args, **kwargs)
def await_task(task): return task.result() if hasattr(task, 'result') else task
def gather_tasks(tasks): return [await_task(t) for t in tasks]
...
__html_buffer = []
def __emit_html(s): __html_buffer.append(s)

def render(obj):
    if hasattr(obj, '__call__'):
        global __html_buffer
        old = __html_buffer; __html_buffer = []; obj(); res = "".join(__html_buffer); __html_buffer = old
        return res
    return str(obj)
...
def __make_tag(tag_name):
    def tag_handler(*args, body=None, **kwargs):
        ...
        for a in args: __emit_html(str(a))
        if body: body()
        if tag_name not in ("meta", "link", "br", "hr", "img", "input"):
            __emit_html(f"</{tag_name}>")
    return tag_handler

html = __make_tag("html"); head = __make_tag("head"); body = __make_tag("body")
div = __make_tag("div"); span = __make_tag("span"); a = __make_tag("a")
img = __make_tag("img"); ul = __make_tag("ul"); ol = __make_tag("ol")
li = __make_tag("li"); button = __make_tag("button"); input = __make_tag("input")
form = __make_tag("form"); table = __make_tag("table"); tr = __make_tag("tr")
td = __make_tag("td"); h1 = __make_tag("h1"); h2 = __make_tag("h2")
h3 = __make_tag("h3"); h4 = __make_tag("h4"); h5 = __make_tag("h5")
h6 = __make_tag("h6"); link = __make_tag("link"); meta = __make_tag("meta")
script = __make_tag("script"); footer = __make_tag("footer"); header = __make_tag("header")
nav = __make_tag("nav"); textarea = __make_tag("textarea"); label = __make_tag("label")
section = __make_tag("section"); article = __make_tag("article"); main = __make_tag("main")
aside = __make_tag("aside"); p = __make_tag("p"); strong = __make_tag("strong")
br = __make_tag("br"); hr = __make_tag("hr"); title = __make_tag("title")
style = __make_tag("style"); blockquote = __make_tag("blockquote")
pre = __make_tag("pre"); code = __make_tag("code")

def std_web_on_request(path, handler, body=None):
    require_web()
    def wrapper():
        if body: return render(body)
...
def std_db_query(query: str, *args):
    require_db()
    conn = _get_db(); cur = conn.cursor(); cur.execute(query, args)
    if query.strip().upper().startswith("SELECT"): return [dict(row) for row in cur.fetchall()]
    conn.commit(); return []
...
def std_db_create_table(model_name: str):
    ...
    if not fields: raise Exception(f"Model '{model_name}' is not defined.")
    cols = []
    for k, v in fields:
        _validate_identifier(k)
        t = "TEXT"
        if v.lower() in ("int", "integer", "bool"): t = "INTEGER"
        elif v.lower() in ("float", "real"): t = "REAL"
        cols.append(f"{k} {t}")
...
def std_db_insert(model_name: str, record: dict):
    require_db()
    _validate_identifier(model_name)
    ks = list(record.keys()); vs = list(record.values())
    for k in ks: _validate_identifier(k)
    std_db_query(f"INSERT INTO {model_name} ({', '.join(ks)}) VALUES ({', '.join(['?']*len(ks))})", *vs)

def _build_where(conds: list):
    if not conds: return "", []
    for k, _, _ in conds: _validate_identifier(k)
...
def std_db_find(model_name: str, conds: list, all: bool = True):
    require_db()
    _validate_identifier(model_name)
    w, v = _build_where(conds); res = std_db_query(f"SELECT * FROM {model_name}{w}", *v)
    return res if all else (res[0] if res else None)

def std_db_update(model_name: str, conds: list, updates: dict):
    require_db()
    _validate_identifier(model_name)
    w, v1 = _build_where(conds); ks = list(updates.keys()); vs = list(updates.values())
    for k in ks: _validate_identifier(k)
...
def std_db_delete(model_name: str, conds: list):
    require_db()
    _validate_identifier(model_name)
    w, v = _build_where(conds); std_db_query(f"DELETE FROM {model_name}{w}", *v)
...
def compress(s: str, t: str):
    ...
    with zipfile.ZipFile(t, 'w', zipfile.ZIP_DEFLATED) as z:
        if os.path.isfile(s): z.write(s, os.path.basename(s))
        else:
            for r, ds, fs in os.walk(s):
                for f in fs: z.write(os.path.join(r, f), os.path.relpath(os.path.join(r, f), os.path.join(s, '..')))
def extract(s: str, t: str):
    require_fs_read(s)
    require_fs_write(t)
    with zipfile.ZipFile(s, 'r') as z: z.extractall(t)
def download(u: str, p: str = None):
    require_net(u)
    if p is None: p = os.path.basename(u)
    require_fs_write(p)
...
def convert(d: Any, f: str):
    if f.lower() == "json": return json.dumps(d)
...
if sys.platform == "win32":
    ...
    def automation_press(key):
        require_automation()
        vk_map = {
            "enter": 0x0D, "tab": 0x09, "space": 0x20, "backspace": 0x08, "shift": 0x10,
            "ctrl": 0x11, "alt": 0x12, "esc": 0x1B, "up": 0x26, "down": 0x28,
            "left": 0x25, "right": 0x27, "delete": 0x2E, "home": 0x24, "end": 0x23,
            "pageup": 0x21, "pagedown": 0x22, "capslock": 0x14, "f1": 0x70, "f2": 0x71,
    ...
    def automation_click(x, y): pass
    def automation_press(key): pass
    def automation_type(text): pass
...
def prompt(msg): return input(f"[PROMPT] {msg}")
def confirm(msg): return input(f"[CONFIRM] {msg} (y/n): ").lower().startswith('y')
def button(l): print(f"[UI] Button: {l}")
def heading(l): print(f"[UI] Heading: {l}")
def paragraph(l): print(f"[UI] Paragraph: {l}")
...
def std_io_read(path): return open(path, "r", encoding="utf-8").read()
def std_io_write(path, content): return open(path, "w", encoding="utf-8").write(content)
def std_io_append(path, content): return open(path, "a", encoding="utf-8").write(content)
def std_io_exists(path): require_fs_read(path); return os.path.exists(path)
def std_io_delete(path): require_fs_write(path); return os.remove(path)
def std_io_copy(src, dest): require_fs_read(src); require_fs_write(dest); return shutil.copy(src, dest)
def std_io_rename(old, new): require_fs_write(old); require_fs_write(new); return os.rename(old, new)
def std_io_mkdir(path): require_fs_write(path); return os.makedirs(path, exist_ok=True)
def std_io_listdir(path): require_fs_read(path); return os.listdir(path)

def close(f): return f.close()
def read(f, n: Optional[int] = None): return f.read() if n is None else f.read(n)
def write(f, data): return f.write(data)
