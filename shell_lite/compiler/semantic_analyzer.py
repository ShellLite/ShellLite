...
class Type:
    ...
    def __repr__(self):
        ...
        if self.class_name: res += f"({self.class_name})"
        if self.element_type: res += f"[{self.element_type}]"
        return res
...
class SemanticAnalyzer(BaseVisitor):
    def __init__(self):
        ...
        builtins = {
            "print": -1, "say": -1, "ask": 1, "len": 1, "str": 1, "int": 1, "float": 1, "bool": 1,
            "abs": 1, "typeof": 1, "range": -1, "max": -1, "min": -1, "add": 2, "remove": 2,
            "xor": 2, "empty": 1, "contains": 2, "split": -1, "upper": 1, "lower": 1, "sort": 1,
            "count": -1, "ord": 1, "char": 1, "json_parse": 1, "json_stringify": -1,
            "pop": -1, "clear_dict": 1, "add": 2, "remove": 2,
            "shl_execute": 1, "shl_parallel": 1,
            "std_io_read": 1, "std_io_write": 2, "std_io_append": 2, "std_io_exists": 1,
            "std_io_delete": 1, "std_io_copy": 2, "std_io_rename": 2, "std_io_mkdir": 1,
            "std_io_listdir": 1, "std_net_get": 1, "std_net_post": 2, "std_web_on_request": -1,
            "std_web_listen": 1, "std_web_serve_static": 2, "std_db_query": -1, "std_db_open": 1,
            "std_db_close": 0, "std_db_exec": -1, "std_db_query_rows": -1, "std_db_model": 2,
            "std_db_create_table": 1, "std_db_insert": 2, "std_db_find": -1, "std_db_update": 3,
            "std_db_delete": 2, "automation_click": 2, "automation_type": 1, "automation_press": 1,
            "automation_notify": 2, "clipboard_copy": 1, "clipboard_paste": 0,
            "html": -1, "head": -1, "body": -1, "div": -1, "span": -1, "a": -1, "img": -1,
            "ul": -1, "ol": -1, "li": -1, "button": -1, "input": -1, "form": -1,
            "table": -1, "tr": -1, "td": -1, "h1": -1, "h2": -1, "h3": -1, "h4": -1,
            "h5": -1, "h6": -1, "link": -1, "meta": -1, "script": -1, "footer": -1,
            "header": -1, "nav": -1, "textarea": -1, "label": -1, "section": -1,
            "article": -1, "main": -1, "aside": -1, "p": -1, "strong": -1, "br": -1, "hr": -1,
            "title": -1, "style": -1, "blockquote": -1, "pre": -1, "code": -1,
            "redirect": 1, "render": 1
        }
    ...
    def analyze(self, ast: List[Node]):
        for stmt in ast:
            if isinstance(stmt, FunctionDef):
                self.global_scope.define(stmt.name, Symbol(stmt.name, SymbolType.FUNCTION, self.T_UNKNOWN, {"arg_count": len(stmt.args)}, is_global=True))
            elif isinstance(stmt, ClassDef):
                self.classes[stmt.name] = stmt
                self.global_scope.define(stmt.name, Symbol(stmt.name, SymbolType.CLASS, Type(TypeKind.OBJECT, stmt.name), {"arg_count": len(stmt.properties)}, is_global=True))
    ...
    def visit(self, node: Node) -> Type:
        if node is None: return self.T_NONE
    ...
    def visit_String(self, node: String) -> Type: return self.T_STR
    def visit_Boolean(self, node: Boolean) -> Type: return self.T_BOOL
        ...
        if not sym:
             for name in self.global_scope.symbols:
                 if name.endswith("_" + node.name):
                     ...
                     break
        if not sym: raise CompileError(f"Undeclared variable '{node.name}'", node.line)
        node.symbol_ref = sym
    ...
    def visit_ListVal(self, node: ListVal) -> Type:
        for e in node.elements: self.visit(e)
    ...
    def visit_ClassDef(self, node: ClassDef) -> Type:
        ...
        for method in node.methods: self.visit(method)
        self.current_scope = prev
        return self.T_NONE
        ...
        if not sym:
             for name in self.global_scope.symbols:
                 if name.endswith("_" + node.class_name):
                     sym = self.global_scope.symbols[name]
                     break
        if not sym: raise CompileError(f"Undeclared class '{node.class_name}'", node.line)
    ...
    def visit_BinOp(self, node: BinOp) -> Type:
        ...
        if node.op == ".":
            if lt.kind not in (TypeKind.OBJECT, TypeKind.UNKNOWN):
                raise CompileError(f"Cannot access member on non-object type", node.line)
        ...
        if node.op in ("+", "-", "*", "/"):
            if lt.kind not in (TypeKind.INT, TypeKind.FLOAT, TypeKind.UNKNOWN) or \
               rt.kind not in (TypeKind.INT, TypeKind.FLOAT, TypeKind.UNKNOWN):
    ...
    def visit_Call(self, node: Call) -> Type:
        ...
        if not sym: raise CompileError(f"Undeclared function '{node.name}'", node.line)
