...
class Parser:
    def __init__(self, source_or_tokens: Any = None, tokens: List[Token] = None):
        ...
        self.precedence = {
            ...
            'IN': 5, 'NOTIN': 5,
            'BIT_OR': 6, 'BIT_XOR': 7, 'BIT_AND': 8,
            'LSHIFT': 9, 'RSHIFT': 9,
            'PLUS': 10, 'MINUS': 10,
            'MUL': 11, 'DIV': 11, 'MOD': 11,
            'POW': 12,
            'DOT': 13, 'LPAREN': 14, 'LBRACKET': 14
        }
        ...
        while i < len(tokens):
            ...
            if t.type == 'NUMBER':
                ...
                while (ops and ops[-1] != 'LPAREN' and
                       (get_precedence(ops[-1]) > get_precedence(t.type) or
                        (get_precedence(ops[-1]) == get_precedence(t.type) and t.type != 'POW'))):
                    apply_op()
                ops.append(t.type)
            i += 1
        while ops:
