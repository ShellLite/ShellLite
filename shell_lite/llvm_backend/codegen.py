...
class LLVMCompiler:
    def __init__(self, base_path=None, filename="main"):
        ...
        self.get_stdin = ir.Function(self.module, gst_ty, name="getchar")
    ...
    def compile(self, statements: List[Node], is_entry_point=False):
    ...
    def _scan_declarations(self, statements: List[Node]):
        for stmt in statements:
    ...
    def visit(self, node: Node) -> Optional[ir.Value]:
    ...
    def visit_Call(self, node: Call) -> Optional[ir.Value]:
        ...
        if node.name == "xor":
            v0 = self.visit(node.args[0])
            v1 = self.visit(node.args[1])
            assert v0 is not None
            assert v1 is not None
            return self.builder.xor(v0, v1, name="xortmp")
        elif node.name == "abs":
    ...
    def visit_IndexAssign(self, node: IndexAssign) -> Optional[ir.Value]:
        ...
        if isinstance(obj_ptr, ir.Constant) and obj_ptr.type == self.char_ptr and obj_ptr.constant is None:
            ...
            self.builder.call(self.dict_set, [obj_ptr, v_idx, v_val])
            return v_val
    ...
    def visit_FileRead(self, node: FileRead) -> Optional[ir.Value]:
        ...
        self.builder.call(self.fread, [buffer, ir.Constant(self.int_type, 1), size, fp])
